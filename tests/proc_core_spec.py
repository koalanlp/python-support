...
def compare_phrase(pytree):
    ...
    if not pytree.isRoot():
        assert pytree.getParent().reference.equals(pytree.reference.getParent())
    else:
        assert pytree.getParent() is None

    term = pytree.getTerminal()
    if term is not None:
        assert term.reference.equals(pytree.reference.getTerminal())
    else:
        assert pytree.reference.getTerminal() is None
