...
def test_read_alphabet():
    ...
    for _ in range(50):
        sample = random.sample(examples, 6)
        original = [t[0] for t in sample]
        korean = [t[1] for t in sample]
        originalStr = "".join(original)
        koreanStr = "".join(korean)
        
        assert ExtUtil.alphaToHangul(originalStr) == koreanStr
        assert ExtUtil.hangulToAlpha(koreanStr) == originalStr
        assert ExtUtil.isAlphaPronounced(koreanStr)

    assert ExtUtil.alphaToHangul("삼성 갤럭시 S9") == "삼성 갤럭시 에스9"
    assert ExtUtil.hangulToAlpha("이마트") == "E마트"
...
def test_translate_hanja():
    ...
    for hanja, hangul in sample:
        assert ExtUtil.hanjaToHangul(hanja) == hangul

    assert ExtUtil.hanjaToHangul("한 女人이 길을 건넜다", False) == "한 녀인이 길을 건넜다"
...
def test_recognize_hanja():
    ...
    assert ExtUtil.isHanja('김') == [False]
...
def test_identify_hangul():
    ...
    assert ExtUtil.isJungsungJamo('\u1161') == [True]

    hangulSentence = "무조건 한글로 말하라니 이거 참 난감하군"
    recognized = ExtUtil.isCompleteHangul(hangulSentence)
    
    for (ch, rec) in zip(hangulSentence, recognized):
        assert (ch != ' ') == rec
        
    mixedSentence = "SNS '인플루엔서' 쇼핑 피해 심각... 법적 안전장치 미비: ㄱ씨는 요즘 ㄴ SNS에서 갤럭시S"
    isCompleteHangul = ExtUtil.isCompleteHangul(mixedSentence)
    isHangul = ExtUtil.isHangul(mixedSentence)
    isIncompleteHangul = ExtUtil.isIncompleteHangul(mixedSentence)
    isChosungJamo = ExtUtil.isChosungJamo(mixedSentence)
    isJungsungJamo = ExtUtil.isJungsungJamo(mixedSentence)
    isJongsungJamo = ExtUtil.isJongsungJamo(mixedSentence)
    
    for ch, a, h, ih, j1, j2, j3 in zip(mixedSentence, isCompleteHangul, isHangul, isIncompleteHangul,
                         isChosungJamo, isJungsungJamo, isJongsungJamo):

        je = ExtUtil.isJongsungEnding(ch)
        ...
        if ch in "SN '.:" :
            ...
            assert not j2
            assert not j3
            assert je == (ch in "인플엔핑심각법적안전장는즘갤럭")

    fragments = mixedSentence.split(" ")
    for fragment in fragments:
        assert ExtUtil.isHangulEnding(fragment) == (fragment not in ("SNS", "'인플루엔서'", "심각...", "미비:", "갤럭시S"))
...
def test_dissemble_hangul():
    ...
    for ch, a, j1, j2, j3 in zip(sent, completeHangul, chosung, jungsung, jongsung):
        if a:
            ...
            assert not any(ExtUtil.isJongsungJamo(j1))

            assert ExtUtil.getChosung(j1)[0] == j1
            assert ExtUtil.getJungsung(j1)[0] is None
            assert ExtUtil.getJongsung(j1)[0] is None

            assert not any(ExtUtil.isChosungJamo(j2))
            assert all(ExtUtil.isJungsungJamo(j2))
            assert not any(ExtUtil.isJongsungJamo(j2))

            assert ExtUtil.getChosung(j2)[0] is None
            assert ExtUtil.getJungsung(j2)[0] == j2
            assert ExtUtil.getJongsung(j2)[0] is None

            if j3 is not None:
                assert not any(ExtUtil.isChosungJamo(j3))
                assert not any(ExtUtil.isJungsungJamo(j3))
                assert all(ExtUtil.isJongsungJamo(j3))
    
                assert ExtUtil.getChosung(j3)[0] is None
                assert ExtUtil.getJungsung(j3)[0] is None
                assert ExtUtil.getJongsung(j3)[0] == j3
        else:
            assert j1 is None
            assert j2 is None
            assert j3 is None
...
def test_assemble_hangul():
    for _ in range(100):
        ...
        assert ExtUtil.getChosung(str) == [t[0] for t in code]
        assert ExtUtil.getJungsung(str) == [t[1] for t in code]
        assert ExtUtil.getJongsung(str) == [t[2] for t in code]
...
""".strip().split("\n")
    ...
    for line in map:
        ...
        for i in range(2, len(splits), 2):
            ...
            for str in eomi.split("/"):
                if str[0] in "ㄴㅂㄹㅁ":
                    testcases.append((root, isVerb, str, result))
                    testcases.append((root, isVerb, ExtUtil.ChoToJong[str[0]] + str[1:], result))
                elif str[0] == 'ㅏ':
    ...
    for verb, isVerb, rest, result in testcases:
        assert ExtUtil.correctVerbApply(verb, isVerb, rest) == result
